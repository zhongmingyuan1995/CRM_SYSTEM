			...
			<div class="col-sm-12">
				<div class="panel">
					<div class="panel-content">
						<div class="row">
							<div class="col-md-12">
								<form class="form-inline" action="queryLikeService.ss?flag=handle" method="post">
						...
						<div class="table-responsive" style="margin-top:30px">
							<table id="basic-table"
								...
								<tbody>
									...
									<%if(service.getSvrStatus().equals("已分配")) {%>
									<tr>
										...
										<td>
											<a  title="处理"href="handleService.ss?svrId=<%=service.getSvrId()%>">
											<i class="fa fa-gg color-primary"></i>&nbsp;处理
											</a>
										</td>										
									</tr>
									<%} }  }%>
							...
							<!--分页-->
								<div class="fenye"  style="float:right">
									<font size="2">共 ${page.totalPageCount} 页</font> <font
										...
										href="queryLikeService.ss?pageNow=1&flag=handle">首页</a>
									<c:choose>
										<c:when test="${page.pageNow - 1 > 0}">
											<a href="queryLikeService.ss?pageNow=${page.pageNow - 1}&flag=handle">上一页</a>
										</c:when>
										<c:when test="${page.pageNow - 1 < 0}">
											<a href="queryLikeService.ss?pageNow=1&flag=handle">上一页</a>
										</c:when>
									</c:choose>
									<c:choose>
										<c:when test="${page.pageNow + 1 <= page.totalPageCount}">
											<a href="queryLikeService.ss?pageNow=${page.pageNow + 1}&flag=handle">下一页</a>
										</c:when>
										<c:when test="${page.pageNow < page.totalPageCount}">
											<a href="queryLikeService.ss?pageNow=${page.totalPageCount}&flag=handle">下一页</a>
										</c:when>
									</c:choose>
									<a href="queryLikeService.ss?pageNow=${page.totalPageCount}&flag=handle">尾页</a>
								</div>	
							<!--分页结束-->
						</div>
					</div>

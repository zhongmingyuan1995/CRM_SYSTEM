			...
			<div class="col-sm-12">
				<div class="panel">
					<div class="panel-content">
						<div class="row">
							<div class="col-md-12">
								<form class="form-inline" action="querysalchanceNodue.do"
									method="post">
									...
									<div class="col-md-12 right-operate">						
										...
										<a href="create_salchance.jsp"class="btn btn-default">
							...
							<table id="basic-table"
								...
								<tbody>
									...
									<tr>
										...
										<td><%=sal.getChcTel()%></td>
										<td><%=sal.getChcStatus()%></td>
										<td>
											<div class="btn-group btn-group-xs">
												<% if(sal.getChcStatus().equals("已指派")){%>
													<a class="btn btn-transparent" title="制定计划" href="queryplan.pl?chcid=<%=sal.getChcId()%>&flag=zdplan">
													<i class="fa fa-pencil-square-o"></i>&nbsp;制定计划
													</a>
												<% }else if(sal.getChcStatus().equals("开发成功")){%>
													<a class="btn btn-transparent" title="查看" href="queryplan.pl?chcid=<%=sal.getChcId()%>&flag=succplan">
													<i class="fa fa-eye"></i>&nbsp;查看
													</a>
												<%}else{%>
													<a class="btn btn-transparent" title="查看" href="queryplan.pl?chcid=<%=sal.getChcId()%>&flag=defeplan">
													<i class="fa fa-eye"></i>&nbsp;查看
													</a>
												<%}%>
											</div>
							...
							<!--分页-->
								<div class="fenye"  style="float:right">
									<font size="2">共 ${page.totalPageCount} 页</font> <font
										...
										href="querysalchanceNodue.do?pageNow=1">首页</a>
									<c:choose>
										<c:when test="${page.pageNow - 1 > 0}">
											<a href="querysalchanceNodue.do?pageNow=${page.pageNow - 1}">上一页</a>
										</c:when>
										<c:when test="${page.pageNow - 1 < 0}">
											<a href="querysalchanceNodue.do?pageNow=1">上一页</a>
										</c:when>
									</c:choose>
									<c:choose>
										<c:when test="${page.pageNow + 1 <= page.totalPageCount}">
											<a href="querysalchanceNodue.do?pageNow=${page.pageNow + 1}">下一页</a>
										</c:when>
										<c:when test="${page.pageNow < page.totalPageCount}">
											<a href="querysalchanceNodue.do?pageNow=${page.totalPageCount}">下一页</a>
										</c:when>
									</c:choose>
									<a href="querysalchanceNodue.do?pageNow=${page.totalPageCount}">尾页</a>
								</div>	
							<!--分页结束-->
						</div>
					</div>
		...
		//设置同步（默认异步true）
			...
			$.get(url,function(data){
	            ...
	            //提交失败的操作  
	            	 ...
	            	 $('#msg').text('删除成功！'); 
	            	 $("#delresult").modal();
	            	 setTimeout("window.location.href='querysalchance.do'",1000);
	         	}  
			});
	} 
</script>  
